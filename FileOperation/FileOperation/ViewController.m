...
- (IBAction)button:(id)sender {
    ...
    //创建文件夹
    if ([WUFileOperation createDirectoryAtPath:folderPath]) {
        NSLog(@"文件夹创建成功");
    ...
    //创建文件
    if ([WUFileOperation createFileAtPath:filePath]) {
        NSLog(@"文件创建成功");
        NSLog(@"文件路径:%@",filePath);
    }else{
        NSLog(@"文件创建失败");
    ...
    //向文件中写入内容
    if ([WUFileOperation writeFileAtPath:filePath content:self.textView.text]) {
        NSLog(@"文件内容写入成功");
